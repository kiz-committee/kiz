...
复合创建对象语法
```
object Name: Parent  # 隐式设置Name.__parent__ = Parent
...
**注意**: 剩余参数以`...`开头, 每个函数只能定义一个剩余参数, 且剩余参数必须放在参数列表的末尾, 
接收剩余参数的变量是一个列表
```
fn function_name(param1, ...rest_param)
    statements
    # 直接通过rest_param这个标识符获取这个列表
end
```
...
注意: ErrorName建议填字符串(也可以填标识符(语法糖))
```
try
    # do something
